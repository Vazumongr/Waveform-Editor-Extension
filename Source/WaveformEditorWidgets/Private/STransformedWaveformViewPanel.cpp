...
void STransformedWaveformViewPanel::Construct(const FArguments& InArgs, const FFixedSampledSequenceView& InData, const TArray<FSoundWaveCuePoint>& InCuePoints, TObjectPtr<USoundWave> InSoundWave)
{
...
void STransformedWaveformViewPanel::UpdateCuePointMarkerPositions(const float PaintedWidth)
{
	...
	SoundWaveToDisplay->GetImportedSoundWaveData(ImportedRawPCMData, ImportedSampleRate, ImportedNumChannels);
	TRange<uint32> displayRange = GridData->GetDisplayRange();
	for (const auto& marker : CuePointMarkers)
	{
		FSoundWaveCuePoint cuePoint = marker->GetCuePoint();
		int32 offset = cuePoint.FramePosition - displayRange.GetLowerBoundValue();
		if (offset < 0)
		{
			marker->SetVisibility(EVisibility::Hidden);
		}
		else
		{
			marker->SetVisibility(EVisibility::Visible);
			float cuePointRatio = static_cast<float>(offset) / static_cast<float>(displayRange.GetUpperBoundValue() - displayRange.GetLowerBoundValue());
			marker->SetMarkerPosition(cuePointRatio * PaintedWidth);
		}
	}
