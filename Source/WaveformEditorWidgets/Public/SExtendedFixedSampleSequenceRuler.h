...
class WAVEFORMEDITORWIDGETS_API SExtendedFixedSampleSequenceRuler : public SCompoundWidget
{
	SLATE_BEGIN_ARGS(SExtendedFixedSampleSequenceRuler)
...
public:
	void Construct(const FArguments& InArgs, TSharedRef<IFixedSampledSequenceGridService> InGridService, TArray<FSoundWaveCuePoint> InCuePoints);
	...
	void SetCuePointMarkerPosition(const int32 InCuePointID, const float InNewPosition);

	void SetMarkerPositions(const TArray<TSharedPtr<SCuePointMarker>>& InMarkers);
