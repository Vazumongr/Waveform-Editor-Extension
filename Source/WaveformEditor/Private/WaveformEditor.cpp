...
void FWaveformEditor::AddCuePoint()
{
	...
	SoundWave->GetImportedSoundWaveData(ImportedRawPCMData, ImportedSampleRate, ImportedNumChannels);
...
bool FWaveformEditor::RegisterToolbar()
{
	...
	if (!UToolMenus::Get()->IsMenuRegistered(MenuName))
	{
		...
		FToolMenuEntry InsertEntry = FToolMenuEntry::InitToolBarButton(
			Commands.InsertCuePoint,
			FText::FromString(TEXT("CuePointEditorInsertCuePointButton")),
			FText::FromString(TEXT("Insert Cue Point")),
			FSlateIcon(FAppStyle::GetAppStyleSetName(), "Icons.Import"));

		InsertEntry.StyleNameOverride = FName("Toolbar.BackplateCenter");

		EditSection.AddEntry(InsertEntry);
